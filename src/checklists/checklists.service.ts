import { Injectable, NotFoundException, OnModuleInit } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { ChecklistTemplate, ChecklistTemplateCategory } from '../common/entities/checklist-template.entity';
import { EquipmentType } from '../common/entities/equipment-type.entity';
import {
  ChecklistCategoryDto,
  CreateChecklistTemplateDto,
  ListChecklistTemplatesQueryDto,
  UpdateChecklistTemplateDto,
} from './dto/admin-checklist.dto';

const DEFAULT_TEMPLATE_CATEGORIES: Record<string, ChecklistTemplateCategory[]> = {
  Elevator: [
    {
      category: 'Machine Room',
      items: ['General condition', 'Traction machine/motor', 'Control panel', 'Electromagnetic brake'],
    },
    {
      category: 'Car',
      items: ['Push buttons', 'Position indicator', 'Door interlock', 'Emergency light'],
    },
    {
      category: 'Hall',
      items: ['Hall buttons', 'Door operation', 'Guide shoes', 'General condition'],
    },
  ],
  Escalator: [
    {
      category: 'Operation',
      items: ['Operating conditions', 'Step and track condition', 'Handrail condition', 'Driving machine'],
    },
    {
      category: 'Safety',
      items: ['Emergency stop button', 'Safety switches', 'Fall prevention fence', 'Skirt guard'],
    },
  ],
  default: [
    {
      category: 'General',
      items: ['Overall operation', 'Safety devices', 'Door and panel condition', 'Abnormal noise/vibration'],
    },
  ],
};

@Injectable()
export class ChecklistsService implements OnModuleInit {
  constructor(
    @InjectRepository(ChecklistTemplate)
    private readonly checklistTemplateRepository: Repository<ChecklistTemplate>,
    @InjectRepository(EquipmentType)
    private readonly equipmentTypeRepository: Repository<EquipmentType>,
  ) {}

  async onModuleInit(): Promise<void> {
    const templateCount = await this.checklistTemplateRepository.count();
    if (templateCount > 0) {
      return;
    }

    const equipmentTypes = await this.equipmentTypeRepository.find({
      where: { isActive: true },
      order: { name: 'ASC' },
    });

    if (equipmentTypes.length === 0) {
      return;
    }

    const defaults = equipmentTypes.map((equipmentType) => {
      const categories = this.cloneCategories(
        DEFAULT_TEMPLATE_CATEGORIES[equipmentType.name] ?? DEFAULT_TEMPLATE_CATEGORIES.default,
      );

      return this.checklistTemplateRepository.create({
        equipmentType,
        name: `${equipmentType.name} Standard Checklist`,
        description: `Baseline inspection template for ${equipmentType.name.toLowerCase()} maintenance visits.`,
        categories,
        isActive: true,
      });
    });

    await this.checklistTemplateRepository.save(defaults);
  }

  private normalizeOptionalText(value?: string | null) {
    const normalized = value?.trim();
    return normalized ? normalized : null;
  }

  private cloneCategories(categories: ChecklistCategoryDto[] | ChecklistTemplateCategory[]) {
    return categories.map((group) => ({
      category: group.category.trim(),
      items: group.items.map((item) => item.trim()).filter(Boolean),
    }));
  }

  private sanitizeCategories(categories?: ChecklistCategoryDto[]): ChecklistTemplateCategory[] {
    return this.cloneCategories(categories ?? []).filter(
      (group) => group.category.length > 0 && group.items.length > 0,
    );
  }

  private async findEquipmentTypeOrThrow(equipmentTypeId: string): Promise<EquipmentType> {
    const equipmentType = await this.equipmentTypeRepository.findOne({
      where: { id: equipmentTypeId },
    });

    if (!equipmentType) {
      throw new NotFoundException('Equipment type not found');
    }

    return equipmentType;
  }

  async findAll(query: ListChecklistTemplatesQueryDto): Promise<ChecklistTemplate[]> {
    const builder = this.checklistTemplateRepository
      .createQueryBuilder('template')
      .leftJoinAndSelect('template.equipmentType', 'equipmentType')
      .orderBy('equipmentType.name', 'ASC')
      .addOrderBy('template.name', 'ASC');

    if (query.equipmentTypeId) {
      builder.andWhere('equipmentType.id = :equipmentTypeId', {
        equipmentTypeId: query.equipmentTypeId,
      });
    }

    if (query.activeOnly === true) {
      builder.andWhere('template.isActive = :isActive', { isActive: true });
    }

    if (query.search?.trim()) {
      builder.andWhere(
        "(LOWER(template.name) LIKE :search OR LOWER(COALESCE(template.description, '')) LIKE :search OR LOWER(equipmentType.name) LIKE :search)",
        { search: `%${query.search.trim().toLowerCase()}%` },
      );
    }

    return builder.getMany();
  }

  async findOne(id: string): Promise<ChecklistTemplate> {
    const template = await this.checklistTemplateRepository.findOne({
      where: { id },
      relations: ['equipmentType'],
    });

    if (!template) {
      throw new NotFoundException('Checklist template not found');
    }

    return template;
  }

  async findPublicTemplate(equipmentTypeName?: string): Promise<ChecklistTemplate | null> {
    const normalizedName = equipmentTypeName?.trim().toLowerCase();
    if (!normalizedName) {
      return null;
    }

    return this.checklistTemplateRepository
      .createQueryBuilder('template')
      .leftJoinAndSelect('template.equipmentType', 'equipmentType')
      .where('template.isActive = :isActive', { isActive: true })
      .andWhere('LOWER(equipmentType.name) = :equipmentTypeName', {
        equipmentTypeName: normalizedName,
      })
      .orderBy('template.updatedAt', 'DESC')
      .getOne();
  }

  async create(payload: CreateChecklistTemplateDto): Promise<ChecklistTemplate> {
    const equipmentType = await this.findEquipmentTypeOrThrow(payload.equipmentTypeId);
    const categories = this.sanitizeCategories(payload.categories);

    const template = this.checklistTemplateRepository.create({
      equipmentType,
      name:
        this.normalizeOptionalText(payload.name) ??
        `${equipmentType.name} Preventive Maintenance Checklist`,
      description: this.normalizeOptionalText(payload.description),
      categories,
      isActive: payload.isActive ?? true,
    });

    return this.checklistTemplateRepository.save(template);
  }

  async update(
    id: string,
    payload: UpdateChecklistTemplateDto,
  ): Promise<ChecklistTemplate> {
    const template = await this.findOne(id);

    if (payload.equipmentTypeId) {
      template.equipmentType = await this.findEquipmentTypeOrThrow(payload.equipmentTypeId);
    }

    if (payload.name !== undefined) {
      template.name =
        this.normalizeOptionalText(payload.name) ??
        `${template.equipmentType.name} Preventive Maintenance Checklist`;
    }

    if (payload.description !== undefined) {
      template.description = this.normalizeOptionalText(payload.description);
    }

    if (payload.categories !== undefined) {
      template.categories = this.sanitizeCategories(payload.categories);
    }

    if (payload.isActive !== undefined) {
      template.isActive = payload.isActive;
    }

    return this.checklistTemplateRepository.save(template);
  }

  async remove(id: string): Promise<void> {
    const template = await this.findOne(id);
    await this.checklistTemplateRepository.remove(template);
  }
}
